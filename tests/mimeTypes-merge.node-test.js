...
export default {
  ...
  if(mergedConfig.js.mime !== 'text/javascript' || mergedConfig.js.encoding !== 'utf8') throw new Error('js mime/encoding merge failed');
  if(mergedConfig.css.mime !== 'text/css' || mergedConfig.css.encoding !== 'utf8') throw new Error('css mime/encoding merge failed');
      ...
      try {
        ...
        if(cssResponse.res.statusCode !== 200){
          throw new Error('CSS file should be served');
        }
        if(cssResponse.res.headers['content-type'] !== 'text/css; charset=utf-8'){
          throw new Error(`CSS should have text/css MIME type with charset, got: ${cssResponse.res.headers['content-type']}`);
        }
        
        const jsResponse = await httpGet(`http://localhost:${port}/test.js`);
        if(jsResponse.res.statusCode !== 200){
          throw new Error('JS file should be served');
        }
        if(jsResponse.res.headers['content-type'] !== 'text/javascript; charset=utf-8'){
          throw new Error(`JS should have custom text/javascript MIME type with charset, got: ${jsResponse.res.headers['content-type']}`);
        }
        
        const htmlResponse = await httpGet(`http://localhost:${port}/test.html`);
        if(htmlResponse.res.statusCode !== 200){
          throw new Error('HTML file should be served');
        }
        if(htmlResponse.res.headers['content-type'] !== 'text/html; charset=utf-8'){
          throw new Error(`HTML should have text/html MIME type with charset, got: ${htmlResponse.res.headers['content-type']}`);
        }
    ...
    await withTempDir(async (dir) => {
      ...
      try {
        ...
        if(jsResponse.res.headers['content-type'] !== 'custom/javascript'){
          throw new Error(`JS should have overridden MIME type custom/javascript, got: ${jsResponse.res.headers['content-type']}`);
        }
        const customResponse = await httpGet(`http://localhost:${port}/test.custom`);
        if(customResponse.res.statusCode !== 200){
          throw new Error('Custom file should be served');
        }
        if(customResponse.res.headers['content-type'] !== 'application/custom'){
          throw new Error(`Custom extension should have application/custom MIME type, got: ${customResponse.res.headers['content-type']}`);
        }
      ...
      try {
        ...
        for(const test of tests){
          ...
          if(response.res.statusCode !== 200){
            throw new Error(`${test.file} should be served (status ${response.res.statusCode})`);
          }
          if(response.res.headers['content-type'] !== test.expectedType){
            throw new Error(`${test.file} should have ${test.expectedType} MIME type, got: ${response.res.headers['content-type']}`);
          }
        }
